        ...
        with self._driver.session() as session:
            ...
            for study in studies.T.to_dict().values():
                ...
                session.write_transaction(
                    node_merging_func, node_type, properties)
                count_node += 1
                if count_node > prev_count_node + 100:
                    prev_count_node = count_node
                    logger.info(
                        "> {} nodes already imported".format(count_node))

        logger.info(
            "> Importing Studies Job is >> Done << with {} nodes imported".format(count_node))
    ...
    def upload_drugs_and_synonyms(self, drug_vocab):
        ...
        with self._driver.session() as session:
            ...
            for key in drug_vocab.keys():
                ...
                drug_id = session.write_transaction(
                    node_merging_func, node_type, properties)
                count_node += 1
                if isinstance(drug_vocab[key], list):
                    for synonym in drug_vocab[key]:
                        ...
                        synonym_id = session.write_transaction(
                            node_merging_func, node_type, properties)
                        count_node += 1
                ...
                if count_node > prev_count_node + 1000 or count_edge > prev_count_edge + 1000:
                    ...
                    logger.info("> {} nodes and {} edges already imported".format(
                        count_node, count_edge))

            logger.info("> Importing Drugs and Synonyms Job is >> Done << with {} nodes and {} edges imported".format(
                count_node, count_edge))
