...
def generate_unwind_property_cypher(properties: list, unwind_iterator_item_name: str) -> str:
    ...
    if properties != [] and properties[0] != {}:
        ...
        for key in properties[0].keys():
            resp += """{key}:{unwind_iterator_item_name}.{key},""".format(
                key=key, unwind_iterator_item_name=unwind_iterator_item_name)
        resp = resp[:-1] + "}"
    return resp
...
class DrugSynonymDataToNeo4j(object):
    ...
    def __init__(self, graph=None, uri="bolt://localhost:7687", user="neo4j", password="letmein", encrypted=False):
        if graph:
            self._driver = graph
        else:
    ...
    def batch_node_merge_handler(self, raw_data, generate_nodes_list, generate_node_data, node_type: str, chunk_size=1000):
        logger.info("Merging '{}' Job is Started to merge {} nodes".format(
            node_type, len(raw_data)))
        ...
        properties = generate_unwind_property_cypher(
            nodes_data, unwind_iterator_item_name="node")
        ...
        with self._driver.session() as session:
            ...
            with Bar("Loading '{}' nodes".format(node_type), fill='@', suffix='%(percent)d%%', max=len(nodes_list)) as bar:
                for i in range(0, len(nodes_data), chunk_size):
                    ...
                    node_ids.extend(session.write_transaction(
                        node_merging_func, node_type, nodes_data_slice, properties))
                    bar.next(chunk_size)

        self.id_store.update(
            {key: value for key, value in zip(nodes_list, node_ids)})

        logger.info("Merging '{}' Job is >> Done << to merge {} nodes".format(
    ...
    def batch_edge_merge_handler(self, raw_data, generate_edge_data, generate_edge_props, edge_type: str, chunk_size=1000):
        logger.info("Merging '{}' Job is Started to merge {} edges".format(
            edge_type, len(raw_data)))
        edge_merging_func = self._batch_merge_edges

        edges_data = generate_edge_data(raw_data, self.id_store)
        properties = generate_unwind_property_cypher(
            generate_edge_props(raw_data), unwind_iterator_item_name="edge")
            ...
            with Bar("Loading '{}' edges".format(edge_type), fill='@', suffix='%(percent)d%%', max=len(edges_data)) as bar:
                for i in range(0, len(edges_data), chunk_size):
                    ...
                    session.write_transaction(
                        edge_merging_func, edge_type, edges_data_slice, properties)
                    bar.next(chunk_size)

        logger.info("Merging '{}' Job is >> Done << to merge {} edges".format(
            edge_type, len(edges_data)))

    def merge_drug_to_synonym_rels(self, drug_synonym_rels):
        self.batch_edge_merge_handler(drug_synonym_rels, self.generate_drug_and_synonym_edge_list_data,
                                      self.generate_drug_and_synonym_edge_props, edge_type="KNOWN_AS")
    ...
    def merge_drugs(self, drug_vocab):
        self.batch_node_merge_handler(
            drug_vocab, self.generate_drug_nodes_list, self.generate_drug_node_data, node_type="Drug")
    ...
    def merge_synonyms(self, drug_vocab):
        self.batch_node_merge_handler(
            drug_vocab, self.generate_synonym_nodes_list, self.generate_synonym_node_data, node_type="Synonym")
    ...
    def _batch_merge_nodes(tx, node_type, nodes_data_slice: dict, properties: str):
        ...
        result = tx.run(cypher_template_filler(
            base_cypher, data), nodes=nodes_data_slice)
    ...
    def _batch_merge_edges(tx, edge_type, edges_data_slice: dict, properties: str, direction=">"):
        ...
        result = tx.run(cypher_template_filler(
            base_cypher, data), edges=edges_data_slice)
        return [int(item["id"]) for item in result]
    ...
    def merge_studies(self, studies: DataFrame):
        self.batch_node_merge_handler(
            studies, self.generate_study_nodes_list, self.generate_study_node_data, node_type="Study")
    ...
    def merge_drug_to_study_rels(self, edges: list):
        self.batch_edge_merge_handler(edges, self.generate_drug_to_study_list_data,
                                      self.generate_drug_to_study_edge_props, edge_type="APPEARED_IN")
    ...
    def merge_url(self, urls: list):
        self.batch_node_merge_handler(
            urls, self.generate_url_nodes_list, self.generate_url_node_data, node_type="Url")
    ...
    def merge_url_to_study_rels(self, edges: list):
        self.batch_edge_merge_handler(edges, self.generate_url_to_study_list_data,
                                      self.generate_url_to_study_edge_props, edge_type="POINTS_AT")
