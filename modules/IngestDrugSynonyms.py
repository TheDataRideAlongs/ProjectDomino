...
class IngestDrugSynonyms():
    ...
    def getAllStudiesByQuery(self, query: str) -> list:
        ...
        from_study = 1
        temp = json.loads(self.loop.run_until_complete(
            asyncio.ensure_future(self.api_wrapper(query, from_study))))
        nstudies = temp['FullStudiesResponse']['NStudiesFound']
        logger.info("{} studies found by '{}' keyword".format(nstudies, query))
        if nstudies > 0:
            self.tasks.extend([asyncio.ensure_future(self.api_wrapper(
                query, study_index)) for study_index in range(from_study, nstudies, 100)])
    ...
    def _convert_US_studies(US_studies: dict) -> pd.DataFrame:
        ...
        for study_list_text in US_studies:
            studies = json.loads(study_list_text)[
            ...
            for temp_full_study in studies:
                ...
                try:
                    temp_dict["intervention"] = temp_study["ArmsInterventionsModule"]["ArmGroupList"][
                        "ArmGroup"][0]["ArmGroupInterventionList"]["ArmGroupInterventionName"][0]
                except:
    ...
    def _scrapeData(self):
        ...
        self.tasks.append(asyncio.ensure_future(
            self.urlToDF(self.url_international)))
        self.tasks.append(asyncio.ensure_future(
            self.urlToDF(self.url_drugbank)))

        for key in self.query_keywords:
            self.getAllStudiesByQuery(key)

        responses: list = self.loop.run_until_complete(
            asyncio.gather(*self.tasks))

        self.internationalstudies: pd.DataFrame = self.xls_handler(
            responses[0])
    ...
    def _filterData(self):
        ...
        self.US_studies_df: pd.DataFrame = self._convert_US_studies(
            self.US_studies)
    ...
    def create_url_study_links(self):
        self.url_points_at_study_edges: list = [(row["study_url"], row["trial_id"], {
        }) for row in self.all_studies_df.to_dict('records')]
